...
| `state_sample_interval_ms` | `1000` | How often each Lua bridge emits `[asobi, lua, state]`. One event per second per bridge, so a world with a hundred live zones is a hundred events per second - lower it only if you are chasing something, raise it if that is more than your sink wants. See [Observability](observability.md) |
